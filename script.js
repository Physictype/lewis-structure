var csvText = `AtomicNumber,Element,Symbol,AtomicMass,NumberofNeutrons,NumberofProtons,NumberofElectrons,Period,Group,Phase,Radioactive,Natural,Metal,Nonmetal,Metalloid,Type,AtomicRadius,Electronegativity,FirstIonization,Density,MeltingPoint,BoilingPoint,NumberOfIsotopes,Discoverer,Year,SpecificHeat,NumberofShells,NumberofValence
1,Hydrogen,H,1.007,0,1,1,1,1,gas,,yes,,yes,,Nonmetal,0.79,2.2,13.5984,8.99E-05,14.175,20.28,3,Cavendish,1766,14.304,1,1
2,Helium,He,4.002,2,2,2,1,18,gas,,yes,,yes,,Noble Gas,0.49,,24.5874,1.79E-04,,4.22,5,Janssen,1868,5.193,1,2
...
75,Rhenium,Re,186.207,111,75,75,6,7,solid,,yes,yes,,,Transition Metal,2,1.9,7.8335,2.10E+01,3453.15,5869,21,Noddack/Berg/Tacke,1925,0.137,6,
...
var csvData = processData(csvText);
...
function getIndex(chemicalSymbol) {
    ...
    for (let i = 0; i < csvData.length; i++) {
        if (csvData[i]["Symbol"]==chemicalSymbol.toString()) {
            return i;
        }
    }
}
function getData(chemicalSymbol,property) {
    if (getIndex(chemicalSymbol) == -1) {
        return "0";
    }
    return csvData[getIndex(chemicalSymbol)][property];
}
function centralComp(a,b) {
    let aElectro = parseInt(getData(a,"Electronegativity"));
    let bElectro = parseInt(getData(b,"Electronegativity"));
    if (a=="H" && b=="H") {
        return 0;
    }
    if (a=="H") {
    ...
    return Math.sign(aElectro - bElectro);
}
function formulaToList(formula) {
    let currentMultiplier = 0;
    let currentChemicalSymbol = "";
    let res = []
    for (let i = 0; i < formula.length; i++) {
        if (/\d/.test(formula[i])) {
            currentMultiplier *= 10;
            currentMultiplier += parseInt(formula[i]);
        } else {
            if (/[A-Z]/.test(formula[i]) && i) {
                currentMultiplier ||= 1
                for (let i = 0; i < currentMultiplier; i++) {
                    res.push(currentChemicalSymbol);
                }
                currentMultiplier = 0;
                currentChemicalSymbol = formula[i];
            } else {
                currentChemicalSymbol += formula[i];
            }
        }
...
function calculateFormalCharge(symbol,lones,bonds) {
    return parseInt(getData(symbol,"NumberofElectrons"))-lones-bonds/2;
}
function sortCentral(formula) {
    return formulaToList(formula).sort(centralComp);
}
...
class LewisStructure {
    ...
    popAtom() {
        this.atoms.pop();
        this.bonds.pop();
        this.loneElectrons.pop();
    }
    getNumBonds(i) {
        let sum = 0;
        for (let j = 0; j < this.bonds.length; j++) {
            if (j == i) {
                sum += this.bonds[j].length;
                continue;
            }
    ...
    totalElectrons() {
        ...
        for (let i = 0; i < this.atoms.length; i++) {
            sum += parseInt(getData(this.atoms[i],"NumberofValence"));
        }
        return sum;
    }
    electronsLeft() {
        let total = this.totalElectrons();
        for (let i = 0; i < this.bonds.length; i++) {
            total -= 2* this.bonds[i].length;
        }
        return total;
    }
    removeAllElectrons() {
        for (let i = 0; i < this.loneElectrons.length; i++) {
            this.loneElectrons[i] = 0;
        }
    }
    atoms;
    bonds;
...
function calculateMaxAbsFormalCharge(structure) {
    ...
    for (let i = 0; i < structure.atoms.length; i++) {
        ...
        let c = Math.abs(valElectrons - structure.loneElectrons[i] - numBonds);
        if (c>charge) {
            charge = c;
        }
    }
